...
.neural-pathway:nth-child(1) { top: 20%; left: 30%; animation-delay: 0s; }
...
/* Enterprise Ecosystem Visualization */
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  class RotatingBanners {
      ...
      this.titleSubtitlePairs = [
        ...
        },
        {
          ...
          subtitles: [
            ...
            "Building systems that learn and evolve",
    ...
    init() {
      ...
      // Start with first subtitle using container fade-in
    ...
    fadeInContainer(container, delay = 0) {
      // Set up the text content first
      this.animateLetters(container, delay);
      
      // Start blurred in
      container.classList.add('blurring-in');
      
      setTimeout(() => {
        // Phase 2: Unblur to normal
        container.classList.remove('blurring-in');
        container.classList.add('normal');
      }, 100); // Quick transition from blurred to normal
    }
    
    updateTitle(newTitle) {
      if (!this.titleElement) return;
      
      // Phase 1: Blur out current title
      this.titleElement.classList.remove('normal');
      this.titleElement.classList.add('blurring-out');
      
      setTimeout(() => {
        // Phase 2: Change text and start blurred in
    ...
    rebuildSubtitleContainers(subtitles) {
      ...
      // Create new containers for current title's subtitles
    ...
    nextBanner() {
      ...
      if (this.currentSubtitleIndex === 0) {
        ...
        this.fadeOutContainer(current, () => {
          ...
          // Fade in new subtitle container and letters
          this.fadeInContainer(this.container.children[0], 0);
          this.isAnimating = false;
        });
    ...
    startRotation() {
      // Subtitle changes every 3 seconds (titles change when cycling through all subtitles)
      setInterval(() => {
        this.nextBanner();
      }, 3000);
    }
